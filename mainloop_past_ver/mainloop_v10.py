...
def handle_click(board, col, row, on_checkbox=False): 
...
"""
checking clicked word vs. correct word/path
"""
#chatgpt was consulted about hamiltonian paths and its syntax
#since yuki who wrote this part has no knowledge about it
def check_clicked_words(board, clicked_cells, all_paths):
    #path = all_paths[0]
    clicked_word = ''.join(board[row][col] for row, col in clicked_cells)
    print('clicked word:', clicked_word)
    print('clicked cell:', clicked_cells)
    if clicked_word in combination:
        print('word match')
    else:
        print('no match')
    #if [cell for cell in clicked_cells] == path[:len(clicked_cells)]:
        #print('clicked cells match')
    #else: 
        #print('clicked cells no match')
    # Check if the clicked word matches the Hamiltonian path
    #if clicked_word == ''.join(board[row][col] for row, col in path[:len(clicked_cells)]):
        # Check if all clicked cells are in the correct sequence according to the Hamiltonian path
        #if all((row, col) == path[i] for i, (row, col) in enumerate(clicked_cells)):
            #return True
    #return False
"""
def check_clicked_words(word, clicked_cells, all_paths, board):
    path = all_paths[0] 
    #lengths do not match -> not correct
    if len(word) != len(clicked_cells):
        return False 
    #for ii in range(len(word)):
        #x, y = path[ii] #coords from hamiltonian path
        #positions do not match
        #if (x, y) not in clicked_cells or word[ii] != board[x][y]:
            #return False
    #return True
    for ii, (x, y) in enumerate(clicked_cells): #path
        if (x, y) not in path:
            return False
        if word[ii] != board[x][y]:
            return False
    return True
        #if (x, y) in path and word[ii] == board[x][y]:
            #continue
        #else:
            #return False
    #return True
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                handle_click(board, col, row, on_checkbox)
    

                
                if on_checkbox:
                    print('clicked!')
                    check_clicked_words(board, clicked_cells, all_paths)
                    #word = ''.join(board[row][col] for row, col in clicked_cells)
                    
                    
                    #if check_clicked_words(word, clicked_cells, all_paths, board):
                        #print(f"Word '{word}' matches Hamiltonian path!")
                    #else:
                        #print(f"Word '{word}' doesn't match Hamiltonian path.")
                    #for word in combination:
                        #if check_clicked_words(word, clicked_cells, all_paths):
                    """
                    valid_sequence = True
                    path = all_paths[0]
                    for i, (row, col) in enumerate(clicked_cells):
                        if (row, col) != path[i]:
                            valid_sequence = False
                            break
                        if valid_sequence:
                            print("All clicked cells form a valid sequence according to the Hamiltonian path.")
                        else:
                            print("Clicked cells do not form a valid sequence according to the Hamiltonian path.")
                          """  
