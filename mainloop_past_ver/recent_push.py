...
#FUNCTION: check if the next move in the Hamiltonian path is a valid move
def is_valid_move(x, y, board_rows, board_cols, visited): 
    return 0 <= x < board_rows and 0 <= y < board_cols and (x, y) not in visited
    #checks if x and y are within the boundaries of the board
    #checks if the coordinate is not visited (if it is valid)

#FUNCTION: to find a valid path
