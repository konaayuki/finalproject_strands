...
# v11 correctly highlights the code yellow but does not hghlight the letters gray
# each time they are clicked! 
# so this version is dedicated to fixing that! 
...
def draw_board(screen, board, theme_cells):
    for row in range(board_rows):
        for col in range(board_cols):
            ...
            # conditionals to highlight the correct color! 
...
def game_over(screen):
    ...
    font = pygame.font.SysFont('any_font', 90)
    text = font.render('game over!', True, BLACK)
    text_rect = text.get_rect(center= (600, 375))
    ...
    if 0 <= row < board_rows and 0 <= col < board_cols: 
        ...
        if last_clicked_cell is None or (
            ...
            if (row, col) == last_clicked_cell: 
                ...
                # clearing clicked_cells for gray highlighting
                clicked_cells = []
